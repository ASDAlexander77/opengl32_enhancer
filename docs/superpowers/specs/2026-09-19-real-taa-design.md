...
Left uncorrected it is not a rounding detail. The Halton `jx` cycle has mean `-0.0547`, so under the history recursion `D <- blend*(D + j)` the image settles into a period-8 oscillation whose **mean displacement is `-0.3099` px** — matching the closed form `blend*mean/(1-blend)` = `0.85 * -0.0547 / 0.15` — **swinging from `+0.01` to `-0.48` px** across the eight frames — the "permanent wobble" this section warns about, produced by single-counting the offset rather than double-counting it. It also blunts the feature itself, since fetching history at the current sample's position re-averages the same scene point instead of accumulating new sub-pixel samples.
