...
// of the fixed post-effect pipeline (see post_effects.cpp), upstream of LUT color grading
// (lut_grading.h). Output stays ordinary 8-bit SDR (this proxy has no way to signal real
...
// Runs the tone-mapping pass on the current back buffer, called from wglSwapBuffers via
// post_effects.cpp. strength is GetAnaxConfig().acesStrength, 0..1: 0 reproduces the input
