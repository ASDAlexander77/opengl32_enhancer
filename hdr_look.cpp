// See hdr_look.h. Single compute dispatch plus a one-float UBO for `strength` (GlComputeApi
// has no glUniform* entry points, so a UBO is used here the same way nis_effect.cpp uses one
// for NISConfig). No capture/blit/state-save of its own - see post_effects.cpp for the shared
// pipeline that owns srcTexture/dstTexture and the app's GL state around the whole chain of
// stages this is one link in.
#include <cstdio>
...
const char* kHdrLookShaderSource =
    ...
    "    float strength;\n"
    "};\n"
    // ACES filmic tone-mapping curve (Narkowicz 2015 fit). acesFilm(0) == 0, so mixing it in
    // by `strength` still leaves strength=0 an exact no-op.
...
bool ApplyHdrLook(unsigned int srcTexture, unsigned int dstTexture, int width, int height, float strength) {
    ...
    if (!g_state.initTried) {
        ...
        if (!g_state.initOk) {
            printf("[opengl32_enh_cpp] hdr_look: shader init failed, effect disabled "
