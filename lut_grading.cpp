// See lut_grading.h. Single compute dispatch plus a GL_TEXTURE_3D built from a parsed .cube
// file. No capture/blit/state-save of its own - see post_effects.cpp for the shared pipeline
// that owns srcTexture/dstTexture and the app's GL state around the whole chain of stages
// this is one link in.
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
bool ParseCubeFile(const char* path, int& outSize, std::vector<float>& outData) {
    ...
    while (fgets(line, sizeof(line), f) != nullptr) {
        ...
        // texture in this codebase (all GL_RGBA8/GL_RGBA16F).
        data.push_back(r);
        data.push_back(g);
        data.push_back(b);
        data.push_back(1.0f);
...
bool ApplyLutGrading(unsigned int srcTexture, unsigned int dstTexture, int width, int height, const char* lutPath, float strength) {
    ...
    if (!g_state.initTried) {
        ...
        if (!g_state.initOk) {
            printf("[opengl32_enh_cpp] lut_grading: shader init failed, effect disabled "
